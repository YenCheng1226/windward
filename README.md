...
| 分頁 | 內容 |
|---|---|
| **行程風險** | 首頁。直接選地點、日期與容許度，得到一段推理：結論、為什麼、每個判斷的依據與把握程度、明確的不確定性聲明、建議行動。附目的地與活動示意圖，可輸出 PNG 與自包含的 HTML 報告 |
...
一個早期版本的實質錯誤：拿離岸 25 km 格點的示性波高，去比對「潛點現場」的門檻。
這兩件事不一樣，而且差很多。

綠島主要潛點（石朗、大白沙）在西南岸。東北向湧浪 2 m 的日子，那一側是背風面，
潛店照常出船——但程式會判定「浪太大不能下水」。

現在分開處理：**船班走外海、沒有島嶼屏蔽，用離岸浪高；潛點在背風側，用扣掉屏蔽的
估計值**。屏蔽係數依週期估算（`shelterFactor`）——短週期風浪擋得住，15 秒以上的長浪
會繞射進背風側，屏蔽效果差很多。一個固定 50% 的規則會把 18 秒的颱風長浪當成跟 8 秒
風浪一樣無害，方向剛好相反。

這是概略估計不是波浪模式，所以離岸值一律同時顯示，而且理由裡會直接寫出來（例如
「浪從東北方來，西南岸背風；外海 2.4 m 到背風側估計剩 1.0 m」）。

### 容許度是設定，不是內建判斷

預設值描述的是謹慎的休閒潛水者。有經驗、或願意接受較差入水條件的人想要更多空間並
沒有錯，所以門檻可以整體縮放：保守 ×0.8、標準 ×1.0、進階 ×1.3。設定會寫進分享網址。
...
報告裡原本放的是颱風雷達圖。它回答「颱風在哪」——只有真的有颱風時才有用。換成目的地
示意圖後回答的是「我該去島的哪一側、每項活動什麼時候最好」，那是每一次都要問的問題。

圖上有：浪從哪個方位打來、哪一岸迎浪、哪一岸背風可以下水、外海與背風側的浪高中位數，
以及五項活動在整段行程中最好的時段。島形是示意，不是實際海岸線。

颱風雷達圖沒有刪掉，移到「熱帶系統」分頁——在那裡它回答的正是該問的問題。
...
| 上游 | GEFS 系集（自算） | 呂宋海峽東側、菲律賓海、關島西方三個生成區的成員最低氣壓 |

只看目的地不夠：1,500 km 外的颱風會用長浪先影響台灣東部，比它自己的氣壓訊號早好幾天，
更遠早於任何機構命名。上游監測就是為了補這個盲點。
