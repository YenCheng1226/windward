...
| 分頁 | 內容 |
|---|---|
| **行程風險** | 一頁看完行程受影響風險：整體評級、逐日風險柱狀圖、熱帶系統相對位置示意圖、風險因素分解、建議行動。可輸出 PNG 示意圖與自包含的 HTML 報告 |
...
`src/lib/risk.ts` 把四條線合成單一風險分數，合法依據的是行程實際失敗的方式：
...
整趟行程的分數取最糟的幾天而非平均——四天裡有一天全毀，才是人們真正記得與需要規劃的。
